...
#create a base raster with 1000x1000, crs = wgs84 and tet extent defined above
r <- raster(crs = p4s_wgs84, wgs_ext, res = 1000)
r[r] <- 1
plot(r)
plot(tet_shp_sp, add = T) #so this isn't showing up
...
all_ras_d <- crop(r, tet_shp_sp) #this does not, but its a square

ocean_ras <- mask(r, land, inverse = T) #not really working either

plot(all_ras_d)
plot(land, add = T) #this is not working either
```
...
r <- fasterize(tet_shp_sp, r, field = "rgn_id")
