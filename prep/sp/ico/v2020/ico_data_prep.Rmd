...
tet_ico_stat_sco <- left_join(tet_ico_list, pop_cat, by = "category") %>%
  mutate(stat_score = case_when(
    category %in% c("LR/lc", "LR/cd") ~ 1.0,
    TRUE ~ stat_score)) %>%
  dplyr::select(-risk_score)

#question - what do we do about low risk conservation dependent? rare?
...
write_csv(tet_ico_stat_sco, file.path(dir_goal, "output/ico_scores.csv"))
