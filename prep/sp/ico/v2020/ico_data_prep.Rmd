...
Bind scinames, rgn_id vector, and eval_year vector to this dataframe
```{r}
...
eval_year <- rep(2019, length(ico_spp_iucn$iucn_sid))
...
tet_ico_full <- tet_ico_stat_sco %>%
  dplyr::select(-sciname, -eval_year) %>%  
  mutate(eval_yr = year) %>% 
  arrange(iucn_sid, year) %>% 
  complete(year = full_seq(year, 1), nesting(iucn_sid)) %>% 
  group_by(iucn_sid) %>% 
  fill(category, stat_score, eval_yr, rgn_id) %>%  ## fills all the way to latest year
  ungroup()

#create
#comname,sciname,ico_gl,ico_rgn_id,iucn_sid,subpop,cat
...
write_csv(tet_ico_stat_sco, file.path(dir_goal, "output/ico_scores.csv"), append = FALSE)
