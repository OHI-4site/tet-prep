...
Adding common names - step one is to read in the spp int file, because we already did that there

```{r}

common_names <- read_csv(here("/prep/bd/spp/v2020/int", "1_iucn_spp_in_tet.csv")) %>% 
...
write_csv(tet_ico_full, file.path(dir_goal, "output/ico_scores.csv"), append = FALSE)

#write to tool box
write.csv(tet_ico_full %>% dplyr::select(region_id = rgn_id, year, sciname, status), file = "~/github/tet-scores/region/layers/ico_status.csv", row.names = FALSE)
```
