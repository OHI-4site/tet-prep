...
Script for initial pull red list categorizations using list from Tetiaroa Society . Thanks to Gage Clawson for helping out with the RRedlist package!
...
```{r}
#try code from global - Sara said it might not work
...
write_csv(tet_ico_hist_all, file.path(dir_goal,'int/tet_ico_history.csv'))
```
...
rgn_id <- rep(1, length(ico_spp_iucn_status$iucn_sid))
eval_year <- rep(2019, length(ico_spp_iucn_status$iucn_sid))
...
#write csv
write_csv(ico_spp_iucn_status, file.path(dir_goal, "output/ico_spp_iucn_status.csv"))
