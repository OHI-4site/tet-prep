...
output:
  html_document:
    highlight: haddock
    includes:
      #in_header: 'Home/github/tet-prep/workflow/templates/ohi_hdr.html' 
    number_sections: yes
    theme: cerulean
    toc: yes
---
...
The highest protection that exists spans half the lagoon and is considered a protected "no-take zone", which is enforced by rangers employed with the Tetiaroa Society. According to the Tetiaroa Society's [sustainable use plan](https://www.tetiaroasociety.org/sites/default/files/research-docs/Tetiaroa-CASUP.pdf), this area is recognized by the Ministry for Natural Resources as of June 2014. Therefore we can consider this area fully protected. 
...
lsp_status_noweight <- all_regions %>% 
  summarize(
    status = mean(status)
  ) #0.525
