...
dir_git       <- '~/github/tet-prep' # Path to the tet-prep github directory
...
plot(raster(slr_nc_files[3]), col = cols, axes = F, 
...
foreach(file = slr_nc_files) %dopar% {
  ...
  r <- raster::crop(r, wgs84) # Current projection is wgs84
...
three_nm <- raster(file.path(dir_git, "spatial/rasters/ocean_ras_100.tif"))
...
foreach(file = annual_means) %dopar% {  
 ...
 # file = annual_means[26]
  ...
  rast_data <- raster(file) %>%
    projectRaster(crs = azim_eq, over = FALSE, progress="text") %>%
...
plot(raster(file.path(dir_anx, "prs_slr/int/msla_annual_us_albers_coastal/msla_annual_azimuthal_coastal_2010.tif")))
