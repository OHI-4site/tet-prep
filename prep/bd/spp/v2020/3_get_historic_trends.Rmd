...
#filter out the ones that have a pop_trend but no calc_trend
...
all_trends <- trends %>% 
  ...
  bind_rows(., lm_trends) %>% 
  inner_join(., class_df, by = "sciname")#yay
...
write.csv(all_trends, file = "~/github/tet-scores/region/layers/spp_trend.csv", row.names = F)
