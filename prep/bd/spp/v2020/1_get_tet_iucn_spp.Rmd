...
write_csv(spp_all, file.path(dir_goal, "/int/2_spp_ico_iucn.csv"))
...
taxa_cells_df_fp <- taxa_cells_list %>%
    bind_rows() %>% 
    filter(!is.na(cell_id)) %>% 
...
Ok so I get 763 species for all of FP, which is A LOT more. Sara's list was ~400 somethings so shouldn't expect to have a majority of these, but 113 still seems low. 

Next step: strategy to work through?
```{r}
