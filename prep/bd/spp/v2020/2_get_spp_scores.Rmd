...
Now need to filter out the subpopulations (which is annoying)
...
test <- spp_class[grepl("South Pacific", spp_class$population) | grepl("Hawaiian", spp_class$population) | grepl("Asia", spp_class$population)| grepl("East Pacific", spp_class$population) | grepl("Indo-west", spp_class$population) | grepl("Oceania", spp_class$population),]

spp_status_final <- spp_class %>% filter(is.na(population)) %>% 
  filter(sciname %!in% test$sciname) %>% 
  bind_rows(., test) %>% 
  dplyr::select(name,class,sciname,category = category.x, stat_score, year)
