...
This script calculates the land habitat score. Because a long term goal for Tetiaroa is to restore abandoned coconut plantation areas to native vegetation, the status score of the land habitat will be based on the extent of coconut grove versus native habitat. We will also include altered habitat (ie landscaped) since much of the habitat on Onetahi has been altered for construction of the Brando. 
...
hab_scores <- tet_hab %>%
  ...
  group_by(habitat) %>% 
  summarize(
    total_hab_area = sum(area_ha)
  ) %>% 
  ungroup() %>% 
  mutate(
    score = case_when(
      habitat %in% "coco_area" ~ 0,
      habitat %in% "native_coco_area" ~ 0.5,
      habitat %in% "native_area" ~ 1,
      habitat %in% "landscape_area" ~ 0.3
    ),
    region_id = 1,
    year = 2020
  )
...
Since this makeup has not changed drastically in the last 5 years, we will simply repeat the above scores for 2014 - 2020.

```{r}

years = rep(seq(2014,2020,1), 5) %>% 
  sort()

land_hab_final <- data.frame(
    region_id = 1,
    habitat = rep(hab_scores$habitat, length(years)),
    hab_area_ha = rep(hab_scores$total_hab_area, length(years)),
    status = rep(hab_scores$score, length(years)),
    year = years
    ) %>% 
  filter(habitat != "total_area")
...
We'll do the actual weighting calculation in the Toolbox. 

```{r}

write_csv(land_hab_final, file.path(dir_goal, "output/land_hab_final.csv"))

#write to toolbox

write.csv(land_hab_final, file = "~/github/tet-scores/region/layers/hab_land_status.csv", row.names = FALSE)
