...
We'll do the actual weighting calculation in the Toolbox. The tricky part is the trend - come back to this. Guessing that the trend for the last 5 years will be pretty close to zero  

```{r}

write_csv(hab_scores %>% filter(habitat != "total_area"), file.path(dir_goal, "output/land_extents.csv"))

#write to toolbox

write.csv(hab_scores %>% filter(habitat != "total_area"), file = "~/github/tet-scores/region/layers/hab_land_status.csv", row.names = FALSE)
